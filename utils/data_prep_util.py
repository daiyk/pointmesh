...
# --------------------------------------------------------------
# Following are the helper function for p2m placeholder preparation
# --------------------------------------------------------------
def load_mesh_data(pkl, params):
        ...
        for i in range(1,4):
                adj = pkl[i][1]
                edges.append(adj[0])
        params.update({'mesh': coord})
        params.update({'edges': [edges[i] for i in range(len(edges))]})
        params.update({'faces': [faces[i] for i in range(len(faces))]})
        params.update({'pool_idx': [pool_idx[i] for i in range(len(pool_idx))]})
        params.update({'lape_idx': [lape_idx[i] for i in range(len(lape_idx))]})
        feed_dict.update({params['support1'][i]: pkl[1][i] for i in range(len(pkl[1]))})
        feed_dict.update({params['support2'][i]: pkl[2][i] for i in range(len(pkl[2]))})
        feed_dict.update({params['support3'][i]: pkl[3][i] for i in range(len(pkl[3]))})
